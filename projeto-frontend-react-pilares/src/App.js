...
import {
  ...
  InputText,
  HeaderCtn2,
} from "./Components/Globals/Containers/styled";
...
function App() {
  ...
  const productTotal = (a, b) => a * b;

  const ordenarCrescente = () =>
    [...dataProducts].sort(
      ({ price: previousID }, { id: currentID }) => previousID - currentID
    );
  ...
  const filterType = (value) => {
    if (Number(value) === 0) {
      ...
      setProducts(
        [...dataProducts].filter(
          (product) => Number(product.typeCategory) === Number(value)
        )
      );
    }
  };
          ...
          <DivTotal>
            ...
            <SectionTitle>
              ...
              <span>
                R${" "}
                {[...carrito].reduce(
                  (productTotal, cartProduct) =>
                    Number(
                      productTotal + cartProduct.price * cartProduct.quantity
                    ),
                  0
                )}
              </span>
              ...
              carrito.map((cartProduct) => (
                <li key={cartProduct.id}>
                  <div>
                    ...
                    <p>
                      R$ {productTotal(cartProduct.quantity, cartProduct.price)}
                    </p>
                  </div>
                </li>
              ))
